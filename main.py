...
from tabulate import tabulate   # to display the sql table neatly
import pickle   # serialize data into files
...
class Component:
    def __init__(self, x, y, width, height, singleInput = False):
        ...
        self.x = x  # Center x
        self.y = y  # Center y
        self.width = width  # width of the component
        self.height = height    #height of the component
        self.colour = (255,0,220)

        self.singleInput = singleInput  #tells wether component has only 1 input (only used for NOT gate and CircuitOutput component)
    ...
    def is_output_hovered(self, mouse_pos): #draws circle for output
    ...
    def drawInputs(self, screen):   # draws the inputs as circles
    ...
    def drawOutputs(self, screen):
        ...
        for compsAndinputs in self.outputs: #draw's a wire as a line from the output to the other components corresponding input
    ...
    def UpdatePos(self, mouse_pos): #update the components position to be where the mouse is
        self.x = mouse_pos[0]
        self.y = mouse_pos[1]
    ...
    def Simulate(self): # changing the input value of the referenced component equal to the output value of this component
...
class CircuitInput(Component):  #it is the component that is itself giving input to the circuit
...
class CircuitOutput(Component): #it is the component that is itself the output of the circuit
...
sqlCon = mysql.connector.connect(host="localhost", user="CircuitSimUser", passwd="SimplePassword", database="SchematicList")
cursor = sqlCon.cursor()
cursor.execute("SELECT * FROM SchematicData")

rows = cursor.fetchall()
column_names = [i[0] for i in cursor.description]  # Fetch column headers

# Display table in a compact format
print("\n\n")
print(tabulate(rows, headers=column_names, tablefmt="simple"))
print("\n\n")



print("Choose wether to create a new Schematic, or open an existing one:")

a = input("""Enter \'e\' for editing schematic, \'n\' for creating new schematic, 
anything else to quit: """)

#wether we are editing an existing schematic or creating a new one
#False means creating a new one, True means editing schematic
SchematicEdit = False

while True:
    if(a[0] == 'e'):
        SchematicEdit = True
        currentSchematic = input("Enter the name of the Schematic you want to Load/Edit: ")
        with open(f"{currentSchematic}.circuit", "rb") as file:
            components = pickle.load(file)
        break
    elif(a[0] == 'n'):
        SchematicEdit = False
        currentSchematic = input("Enter the name of the Schematic you want to create: ")
        with open(f"{currentSchematic}.circuit", "wb") as file:
            pass
        break
    else:
        sqlCon.close()
...
while True:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            numInputs = sum(1 for element in components if isinstance(element, CircuitInput))
            numOutputs = sum(1 for element in components if isinstance(element, CircuitOutput))
            totalComponents = len(components)

            #overwrite the previous data with new data
            with open(f"{currentSchematic}.circuit", "wb") as file:
                pickle.dump(components, file)

            if(SchematicEdit):
                update_query = """
                    UPDATE SchematicData 
                    SET number_of_components = %s, number_of_inputs = %s, number_of_outputs = %s
                    WHERE schematic_name = %s
                """
                cursor.execute(update_query, (totalComponents, numInputs, numOutputs, currentSchematic))
            else:
                query = """
                    INSERT INTO SchematicData (schematic_name, number_of_components, number_of_inputs, number_of_outputs)
                    VALUES (%s, %s, %s, %s)
                """
                values = (currentSchematic, totalComponents, numInputs, numOutputs)
